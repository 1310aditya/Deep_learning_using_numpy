...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            dwords, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, h_cache)
            ...
            dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dout, scores_cache)
            dwords, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, h_cache)
            grads['W_embed']= word_embedding_backward(dwords, words_cache)
            dfeatures, grads['W_proj'], grads['b_proj'] = affine_backward(dh0, h0_cache)
        ############################################################################
        #                             END OF YOUR CODE                             #
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'rnn':
            h0, h0_cache = affine_forward(features, W_proj, b_proj)
            word = np.array([self._start for n in range(N)])
            prev_h = h0
            for t in range(max_length):
                word, word_cache = word_embedding_forward(word, W_embed)
                prev_h, rnn_cache = rnn_step_forward(word, prev_h, Wx, Wh, b)
                scores, scores_cache  = affine_forward(prev_h, W_vocab, b_vocab)
            ...
            for t in range(max_length):
                word, word_cache = word_embedding_forward(word, W_embed)
                prev_h, prev_c, lstm_cache = lstm_step_forward(word, prev_h, prev_c, Wx, Wh, b)
                scores, scores_cache  = affine_forward(prev_h, W_vocab, b_vocab)
                word = np.argmax(scores, axis = 1)
                captions[:, t] = word
        ############################################################################
