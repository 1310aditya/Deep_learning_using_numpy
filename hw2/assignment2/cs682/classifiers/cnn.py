...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, affine_cache = affine_forward(h_out, W3, b3)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        #dout = scores
        da, dw, db = affine_backward(dout, affine_cache)
        grads['W3'] = dw
        grads['b3'] = db
        dh, dw, db = affine_relu_backward(da, h_cache)
        grads['W2'] = dw
        grads['b2'] = db
        dx, dw, db = conv_relu_pool_backward(dh, conv_cache)
        grads['W1'] = dw
        grads['b1'] = db
        loss += 0.5*self.reg*np.sum([np.sum(self.params[f'W{i}']**2) for i in range(1,4)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
